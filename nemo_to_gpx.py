...
def distanceGPS(latA, longA, latB, longB):
    """Retourne la distance en mètres entre les 2 points A et B connus grâce à
       leurs coordonnées GPS (en degrés décimaux).
    """
...
def nemo_to_gpx(start_date: datetime, end_date: datetime, delta_time_minutes: int = 0, file_name: str = 'output.gpx'):
    ...
    if data['data']:
        ...
        for it_response in data['data']:
            ...
            longB=it_response['loc'][0]                    
            latB=it_response['loc'][1]            
            #Compute distance between two last points after first point
            if (n_index>2 and float(it_response['speed']) != 0 and not(longA==longB and latA==latB)):
                dist = distanceGPS(latA, longA, latB, longB)/1852
                format_dist = "{:.3f}".format(dist)
                total_dist += dist
                
            n_index+=1
            longA=it_response['loc'][0]                    
            latA=it_response['loc'][1]
        # Define gpx output file
        with open(file_name, 'w') as f:
            f.write(gpx_data.to_xml())
        print(f'Created GPX file "{file_name}" with {n_points_out} points')
        
        #Print average speed in kts (*1,945 => Pourquoi ce coef et non pas 1,852 ? sur fishweb-nemo, la vitesse est en noeuds, alors que la vitesse renvoyée est dans une unité inconnue, donc coef trouvé empiriquement... )
        ...
        #Print total distance in nautic miles
